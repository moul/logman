...
func TestLogfile(t *testing.T) {
	...
	{
		files, err := logman.LogfileList(filepath.Join(tempdir, "doesnotexist"))
		require.Error(t, err)
		require.Nil(t, files)
	}
	...
	{
		files, err := logman.LogfileList(tempdir)
		require.NoError(t, err)
	...
	{
		writer, err := logman.NewWriteCloser(tempdir, "kind-1")
		require.NoError(t, err)
	...
	{
		for i := 0; i < 10; i++ {
			writer, err := logman.NewWriteCloser(tempdir, fmt.Sprintf("hello-%d", i))
			require.NoError(t, err)
			err = writer.Close()
			require.NoError(t, err)
		}
